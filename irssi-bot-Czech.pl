...
sub add_admin {
    ...
    if (!grep { $_ eq $addr } @admin_hosts) {
        ...
        save_admins();
        $server->command("/msg $chan Admin přidán: $addr");
    } else {
        $server->command("/msg $chan $addr už je admin.");
    }
}
...
sub del_admin {
    ...
    save_admins();
    $server->command("/msg $chan Admin odebrán: $addr");
}

sub list_admins {
    my ($server, $chan) = @_;
    my $list = join(", ", @admin_hosts);
    $server->command("/msg $chan Aktuální admini: $list");
}
...
sub cmd_version {
    ...
    $server->command("/msg $chan VPS verze: $version");
}
...
sub cmd_help {
    ...
    $server->command("/msg $chan Dostupné příkazy: $help_msg");
}

# === KEEP NICK ===
sub start_keepnick {
    my ($nick, $server, $chan) = @_;
    stop_keepnick();  # Nejprve zastavit, pokud běží
    ...
    $server->command("/msg $chan Keepnick aktivní pro nick: $keepnick_nick");
}
...
sub handle_command {
    ...
    if ($msg =~ /^!op (\S+) (\S+)/) {
        ...
        stop_keepnick();
        $server->command("/msg $default_chan Keepnick deaktivován.");
    }
}

# === Funkce pro jednotlivé příkazy ===
sub cmd_deop {
    my ($server, $channel, $nick) = @_;
    $server->command("MODE $channel -o $nick");
}
...
sub cmd_host {
    my ($server, $ip, $chan) = @_;
    my $result = `host $ip`;  # Spustí whois na ip adresu
    chomp($result);
    $server->command("/msg $chan Host výsledek: $result");
}
...
sub cmd_nick {
    ...
    if ($newnick =~ /^[a-zA-Z0-9_\-\[\]\\\^\{\}\|`]+$/) {
        ...
        $server->command("/msg $chan Nick změněn na: $newnick");
    } else {
        $server->command("/msg $chan Neplatný nick: $newnick");
    }
}
# === Registrace signálů ===
...
# === Inicializace ===
load_admins();
Irssi::print("Bot script načten – !help a další.");
